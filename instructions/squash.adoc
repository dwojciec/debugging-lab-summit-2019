...
directory in the **Terminal Window of Che7**:

In Che7, `*use the Commands Palette and click on 'DEBUG > Squash Version'*`

[source,shell]
----
Squash version used from Solo.io
squashctl version 0.5.15, created 2019-05-14.23:26:51
----

The Java image on OpenShift has built-in support for remote debugging and it can be enabled by setting the **JAVA_DEBUG=true** 
environment variables on the deployment config for the pod that you want to remotely debug.
In the **Terminal Window of Che7**, `*execute the following command*`:
...
Using **squashctl** on the **Terminal Window of Che7** or you can use the **Command** **Debug Squash Inventory**
...
=== Remote Debug with Che7

Che7 provides a convenient way to remotely connect to Java applications running 
...
The window shows the debuggers available in Che7. Click on the plus sign near the 
Java debugger.
...
You can now click on the **Debug** button to make Che7 connect to the 
...
Open a new **Terminal Window of Che7** and use `curl` to invoke the Inventory API with the 
...
Go back to the **Terminal Window of Che7**. Press 
...
`*run 'Task: Run Task...' ->  'che: Build and deploy Inventory Service'*`
